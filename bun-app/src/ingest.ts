...
export function ingest(events: Record<string, unknown>[]): void {
  for (const raw of events) {
    ...
    // TYPING: store a normalized interpretation (no LLM). Others: placeholder until LLM overwrites.
    const interpretation =
      event_type === "TYPING" ? `typed '${detail ?? ""}'` : quickInterpretation(event_type, app, detail);
