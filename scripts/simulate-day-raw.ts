 ...
 * simulate-day-raw.ts
 *
 * Same as simulate-day.ts but passes raw events (event_type + app + detail)
 * to the agent instead of pre-interpreted sentences. The agent receives the
 * same event-decoding context from interpret_events.md and must both
 * understand and group the events in one pass.
 *
 * Outputs:
 *   scripts/simulate-day-raw.html
 *   scripts/simulate-day-raw.json
 *
 * Usage:
 *   bun run scripts/simulate-day-raw.ts [--date=YYYY-MM-DD]
 */
...
function loadTodayEvents(): RawEventRow[] {
  ...
  // Load events that had interpretations in the real DB (same population as simulate-day.ts)
  const rows = realDb.prepare(`
    SELECT id, timestamp, app, event_type, detail
    FROM raw_events
...
function buildSimDb(events: RawEventRow[]): Database {
  ...
  const insert = sim.prepare(
    "INSERT INTO raw_events (id, timestamp, app, event_type, detail) VALUES (?, ?, ?, ?, ?)"
  );
  const insertAll = sim.transaction((rows: RawEventRow[]) => {
    for (const r of rows) insert.run(r.id, r.timestamp, r.app, r.event_type, r.detail);
  });
  insertAll(events);
  return sim;
}
...
// ── Raw event formatter ───────────────────────────────────────────────────────
// Converts raw detail JSON into a compact, readable string for the agent.

function formatDetail(eventType: string, detail: string | null): string {
  if (!detail) return "";

  // Plain-text event types — show as-is
  if (["TYPING", "KEY", "PASTE", "COPY", "SHORTCUT", "APP SWITCH"].includes(eventType)) {
    return detail.slice(0, 150);
  }

  // CLICK / SCROLL — extract the meaningful fields from JSON
  try {
    const d = JSON.parse(detail);
    const parts: string[] = [];

    // What was clicked/scrolled on
    const t = d.target;
    if (t) {
      const label = t.label || t.title || t.value || t.description;
      if (label && String(label).trim()) parts.push(`target="${String(label).slice(0, 80)}"`);
      else if (t.role) parts.push(`target=[${t.role}]`);
    }

    // Where it happened — URL or page title from context
    if (Array.isArray(d.context)) {
      for (const ctx of d.context) {
        if (ctx.document && ctx.document.length < 120) {
          parts.push(`url=${ctx.document}`);
          break;
        }
        if (ctx.title && String(ctx.title).length < 60) {
    ...
    // Nearby items — first 4 siblings for positional context
    if (Array.isArray(d.siblings) && d.siblings.length > 0) {
      const sib = d.siblings
        .slice(0, 4)
        .map((s: Record<string, unknown>) => s.label || s.title || s.value)
        .filter(Boolean)
        .join(", ");
      if (sib) parts.push(`near=[${String(sib).slice(0, 80)}]`);
    }

    const out = parts.join(" | ");
    return out.length > 0 ? out : detail.slice(0, 150);
  } catch {
    return detail.slice(0, 150);
  }
}
  ...
  const TOOLS: Anthropic.Tool[] = [
    {
      name: "query_events",
      description:
        "Read recent activity events from the database. Returns each event's id, timestamp, app, event_type, raw detail, and task_id (null = unassigned). Use your knowledge of macOS event types to understand what the user was doing.",
      input_schema: {
    ...
    },
    {
      ...
      input_schema: {
        type: "object" as const,
        properties: {
          title: { type: "string", description: "Short goal statement (6-12 words). Use specific names — files, URLs, people — when visible in the events." },
...
const SYSTEM_PROMPT = `You are a background task segmentation agent. Your job is to look at a user's raw macOS activity events and group them into tasks — either existing ones or new ones.

The events come from the macOS Accessibility API and CGEvent tap. You must interpret what the user was doing from the raw data and group them accordingly.

## Reading raw events

Each event line looks like:
  id=N HH:MM [unassigned|task:ID] AppName | EVENT_TYPE | <detail>

**Event types:**
- APP SWITCH — switched to a different app. detail = app name
- CLICK — clicked something. detail includes: target="label" url=... near=[siblings]
- SCROLL — scrolled. detail includes: target, url/page context
- TYPING — buffered keystrokes. detail = the typed text
- KEY — special key press (Enter, Escape, Tab, Backspace, arrow). detail = key name
- PASTE — pasted clipboard. detail = pasted text (truncated)
- COPY — copied text. detail = copied text
- SHORTCUT — keyboard shortcut (e.g. Cmd+s, Cmd+z). detail = shortcut

**Reading CLICK detail:**
- target="..." — what was clicked (label, button text, item title)
- url=... — the page URL in the browser at time of click
- page="..." — the window or page title
- near=[...] — nearby sibling elements (gives positional context)

**Common patterns:**
- TYPING followed by KEY (Enter) = typed and submitted something
- APP SWITCH to browser + CLICK + TYPING = web search or form entry
- SHORTCUT Cmd+s = saved a file
- SHORTCUT Cmd+z = undo
- Repeated CLICKs in same app = navigating within that app
- APP SWITCH back and forth = interleaving two tasks
...
2. APP SWITCHES ARE NOT SPLITS: browser ↔ IDE ↔ terminal while working toward the same goal = ONE task.
3. MULTI-STEP WORKFLOWS = ONE TASK: git init + gh auth + OAuth flow = one task.
...
async function runWindow(
  ...
  const unassigned = (sim.prepare(`
    ...
    WHERE task_id IS NULL AND timestamp >= ? AND timestamp <= ?
...
// ── HTML generation (identical to simulate-day.ts) ────────────────────────────

interface TaskRow { id: number; title: string; description: string; category: string | null; }
interface EventOut {
  event_id: number; timestamp: string; app: string; event_type: string;
  detail: string | null; task_id: number; task_title: string;
  task_description: string; category: string | null;
}

function generateHtml(tasks: TaskRow[], events: EventOut[], date: string): string {
...
<title>Activity Simulation (raw) — ${date}</title>
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .bar-col { flex: 1; height: 100%; display: flex; align-items: flex-end; transition: opacity 0.15s; }
...
<body>
<h1>Activity Simulation (raw events) — ${date}</h1>
<p class="subtitle">Agent received raw event data only — no pre-interpreted sentences</p>
<div class="layout">
  <div class="left">
...
const CATEGORY_COLORS = {
  Productivity: "#0a84ff", Leisure: "#30d158", Admin: "#ff9f0a",
  Learning: "#bf5af2", Communication: "#32ade6", Uncategorized: "rgba(255,255,255,0.12)",
};
...
let hoveredTaskId = null, selectedTaskId = null, selectedBarIndex = null;
...
function renderChart() {
  ...
  buckets.forEach((bucket, i) => {
    ...
    col.className = "bar-col"; col.style.opacity = colOpacity;
    const stack = document.createElement("div");
    stack.className = "bar-stack"; stack.style.height = barHeight + "%";
    bucket.tasks.forEach((task, ti) => {
      ...
      seg.addEventListener("mouseenter", e => { hoveredTaskId = task.taskId; showTooltip(task, e.clientX, e.clientY); renderChart(); });
      seg.addEventListener("mousemove", e => moveTooltip(e.clientX, e.clientY));
      seg.addEventListener("mouseleave", () => { hoveredTaskId = null; hideTooltip(); renderChart(); });
      seg.addEventListener("click", () => {
        if (selectedTaskId === task.taskId && selectedBarIndex === i) { selectedTaskId = null; selectedBarIndex = null; closeDetail(); }
        else { selectedTaskId = task.taskId; selectedBarIndex = i; showDetail(task.taskId, i); }
        renderChart(); updateTaskList();
      });
      stack.appendChild(seg);
    });
    col.appendChild(stack); chart.appendChild(col);
    const xl = document.createElement("div");
    xl.className = "x-label"; xl.textContent = bucket.hour % 3 === 0 ? label : ""; xaxis.appendChild(xl);
  });
}
...
function showTooltip(task, x, y) {
  ...
  document.getElementById("tt-time").textContent = events.filter(e => e.task_id === task.taskId).length + " events";
  tt.style.left = Math.min(x + 12, window.innerWidth - 240) + "px";
  tt.style.top = (y - 8) + "px"; tt.style.display = "block";
}
function moveTooltip(x, y) {
  const tt = document.getElementById("tooltip");
  tt.style.left = Math.min(x + 12, window.innerWidth - 240) + "px"; tt.style.top = (y - 8) + "px";
}
...
function showDetail(taskId, barIndex) {
  ...
  const matching = events.filter(ev => { const ts = new Date(ev.timestamp).getTime(); return ev.task_id === taskId && ts >= bucket.startMs && ts < bucket.endMs; });
  const container = document.getElementById("detail-events"); container.innerHTML = "";
  for (const ev of matching) {
    const row = document.createElement("div"); row.className = "ev-row";
    row.innerHTML = \`<span class="ev-time">\${formatTime(ev.timestamp)}</span><span class="ev-type" style="color:\${EVENT_TYPE_COLORS[ev.event_type]||'var(--text3)'}">\${ev.event_type.toLowerCase()}</span><span class="ev-app">\${ev.app}</span><span class="ev-detail">\${ev.detail || "—"}</span>\`;
    container.appendChild(row);
  }
}

function closeDetail() { document.getElementById("detail").style.display = "none"; selectedTaskId = null; selectedBarIndex = null; renderChart(); updateTaskList(); }
...
function renderStats() {
  document.getElementById("stats").innerHTML = \`<div class="stat"><b>\${tasks.length}</b> tasks</div><div class="stat"><b>\${events.length}</b> events assigned</div><div class="stat"><b>\${date}</b></div>\`;
}
...
function updateTaskList() {
  ...
  for (const task of sortedTasks) {
    ...
    item.innerHTML = \`<div class="task-item-title">\${task.title}</div><div class="task-item-meta"><div class="dot" style="background:\${CATEGORY_COLORS[cat]}"></div><span>\${cat}</span><span style="margin-left:auto">\${evCount} events</span>\${firstTs?'<span>'+formatTime(firstTs)+'</span>':''}</div>\`;
    item.addEventListener("click", () => {
      if (selectedTaskId === task.id) { selectedTaskId = null; selectedBarIndex = null; closeDetail(); }
      else {
        let bestBar = 0, bestCount = 0;
        buckets.forEach((b, i) => { const t = b.tasks.find(t => t.taskId === task.id); if (t && t.count > bestCount) { bestCount = t.count; bestBar = i; } });
        selectedTaskId = task.id; selectedBarIndex = bestBar;
        showDetail(task.id, bestBar); renderChart(); updateTaskList();
      }
    });
...
renderStats(); renderChart(); renderLegend(); updateTaskList();
</script>
</body>
</html>`;
}
...
async function main() {
  console.log(`simulate-day-raw: loading events for ${TARGET_DATE}…`);
  const events = loadTodayEvents();
  if (events.length === 0) { console.error(`No events found for ${TARGET_DATE}`); process.exit(1); }
  console.log(`simulate-day-raw: ${events.length} events loaded`);
  ...
  while (t <= lastMs + intervalMs) { windows.push(t); t += intervalMs; }

  console.log(`simulate-day-raw: running ${windows.length} windows…`);
  ...
  const tasks = sim.prepare("SELECT id, title, description, category FROM tasks ORDER BY id ASC").all() as TaskRow[];
  const eventRows = sim.prepare(`
    SELECT e.id as event_id, e.timestamp, e.app, e.event_type, e.detail,
  ...
  const unassigned = (sim.prepare("SELECT COUNT(*) as n FROM raw_events WHERE task_id IS NULL").get() as { n: number }).n;

  console.log(`\nsimulate-day-raw: done`);
  console.log(`  tasks created: ${tasks.length}`);
  console.log(`  events assigned: ${eventRows.length} / ${events.length}`);
  console.log(`  unassigned: ${unassigned}`);

  const outDir = join(import.meta.dir);
  writeFileSync(join(outDir, "simulate-day-raw.json"), JSON.stringify({ date: TARGET_DATE, tasks, events: eventRows }, null, 2));
  writeFileSync(join(outDir, "simulate-day-raw.html"), generateHtml(tasks, eventRows, TARGET_DATE));

  console.log(`\n  → ${join(outDir, "simulate-day-raw.json")}`);
  console.log(`  → ${join(outDir, "simulate-day-raw.html")}`);
}

main().catch(e => { console.error("simulate-day-raw: failed", e); process.exit(1); });
